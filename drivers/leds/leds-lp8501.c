...
struct lp8501_led_config *g_led_config_dbg;
int g_led_config_num = 0;

void lp8501_debug_led(int index, int percent)
{
	struct lp8501_led_config *led_config;
	struct lp8501_device_state *state;
	int pwm_value, i;

	if (g_led_config_dbg == NULL || g_led_config_num == 0)
		return;

	if (index < 0 || index >= g_led_config_num)
		return;

	if(convert_percent_to_pwm(percent, &pwm_value) != 0)
		return;

	led_config = &(g_led_config_dbg[index]);
	state = i2c_get_clientdata(led_config->client);
		...
		const struct i2c_device_id *id)
{
	...
	g_led_config_dbg = leds;
	g_led_config_num = pdata->nleds;
